...
public class PictureService {
    ...
    public Mono<Picture> fetchAndSavePicture(String animal, String jobId) {
        ...
        switch (animal.toLowerCase()) {
            case "dog":
                pictureDataMono = externalApiService.fetchDogPicture();
                LOGGER.info("##### API response for the animal = {} ", animal);
                break;
            case "bear":
                pictureDataMono = externalApiService.fetchBearPicture();
                LOGGER.info("##### API response for the animal = {} ", animal);
                break;
            default:
    ...
    public Mono<Picture> getPictureByJobId(String jobId) {
        return pictureRepository.findByJobId(jobId);
    }
